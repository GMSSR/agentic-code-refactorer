...
from src.prompt import eval_prompt, j_eval_prompt, ref_prompt, j_ref_prompt
from src.llm import unified_call
from src.schemas import Evaluation, JudgementE, Refactor, JudgementR
...
def eval_func(old_eval): # likely should be moved to another file
    ...
    evaluation = unified_call(prompt=eval_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, previous_eval=evaluation, feedback=judgment.get('feedback')), model=container.config.eval_model, schema=Evaluation)
...
def judge_eval_func(eval): # likely should be moved to another file
    ...
    judgment = unified_call(prompt=j_eval_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, eval=evaluation), model=container.config.j_eval_model, schema=JudgementE)
...
def proposal_func(old_proposal): # likely should be moved to another file
    ...
    proposal = unified_call(prompt=ref_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, eval=evaluation, previous_proposal=proposal, feedback=judgment.get('feedback')), model=container.config.ref_model, schema=Refactor)
...
def judge_proposal_func(refactor): # likely should be moved to another file
    ...
    judgment = unified_call(prompt=j_ref_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, eval=evaluation, ref=proposal), model=container.config.j_ref_model, schema=JudgementR)
...
if __name__ == "__main__": #needed due to using ProcessPoolExecutor on static_ana
    ...
    if start_index <= c.STAGES.index('static_analysis'):
        ...
        for smell_type, smell in code_smells:
            ...
            if heuristics == "N":
                ...
                continue
            evaluation = unified_call(prompt=eval_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell), model=container.config.eval_model, schema=Evaluation)
            temp.append([smell_type, smell, heuristics, evaluation])

        for smell_type, smell, heuristics, evaluation in temp: # This likely can be easily offloaded to feedback_loop, remember to change save_checkpoint to support
            judgment = unified_call(prompt=j_eval_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, eval=evaluation), model=container.config.j_eval_model, schema=JudgementE)
        ...
        save_checkpoint(current_stage, approved_e=approved_eval, rejected_e=rejected_eval, approved_p=approved_proposal, rejected_p=rejected_proposal, output=container.state.output, code_path=container.config.code_path)
        if inter:
            sys.exit(130)
    ...
    if start_index <= c.STAGES.index('eval_loop'):
        ...
        new_approved_eval, rejected_eval = feedback_loop(rejected_eval, process_func=eval_func, judge_func=judge_eval_func)
    ...
    if start_index <= c.STAGES.index('initial_refactoring') and not container.config.is_eval_only:
        temp = []
        for smell_type, smell, heuristics, evaluation in approved_eval:
            proposal = unified_call(prompt=ref_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, eval=evaluation), model=container.config.ref_model, schema=Refactor)
            temp.append([smell_type, smell, heuristics, evaluation, proposal])

        for smell_type, smell, heuristics, evaluation, proposal in temp:
            judgment = unified_call(prompt=j_ref_prompt(type_smell=smell_type, heuristics=heuristics, smell=smell, eval=evaluation, ref=proposal), model=container.config.j_ref_model, schema=JudgementR) # This likely can be easily offloaded to feedback_loop, remember to change save_checkpoint to support
    ...
    if start_index <= c.STAGES.index('refactoring_loop') and not container.config.is_eval_only:
        ...
        new_approved_proposal, rejected_proposal = feedback_loop(rejected_proposal, process_func=proposal_func, judge_func=judge_proposal_func)

        approved_proposal.extend(new_approved_proposal)

        for smell_type, smell, heuristics, evaluation, proposal, judgment in rejected_proposal:
            container.state.output.append({
                "smell_type": smell_type,
                "smell": smell,
                "evaluation": evaluation,
    ...
    print(f"The number of smells that weren't able to have an evaluation approved within the maximum number of attempts is {len(rejected_eval)}.")
    if not container.config.is_eval_only: print(f"The number of smells that weren't able to have a refactoring proposal approved within the maximum number of attempts is {len(rejected_proposal)}.")
